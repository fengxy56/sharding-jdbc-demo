...
public interface CommonMapper {
    ...
    int addTOrderItem(TOrderItem tOrderItem);

    @Insert("insert into t_order(order_id,order_content) values(#{order_id},#{order_content})")
    int addTOrderAndId(TOrder order);

    @Select("select * from t_order a,t_order_item b where a.order_id=b.order_id and a.order_id=#{order_id}")
    List<Map<String,Object>> queryTOrderById(TOrder order);

    @Select("select * from t_order a,t_order_item b,t_dict c where a.order_id=b.order_id and b.order_id=c.order_id and a.order_id=#{order_id}")
    List<Map<String,Object>> queryTOrderItem(TOrder order);
