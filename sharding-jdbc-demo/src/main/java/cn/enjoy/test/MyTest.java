...
import cn.enjoy.bean.TOrderItem;
import cn.enjoy.util.SnowflakeUtil;
import com.alibaba.fastjson.JSONObject;
import cn.enjoy.ShardingApplication;
...
public class MyTest {
    ...
    * */
    ...
    public void test1() {
        ...
        order.setOrder_id(146);
        order.setOrder_content("jack146");
        commonMapper.addT_Order(order);
    }
    @Test
    public void testAddTOrderItem() {

        for(int i=100;i<200;i++){
            int orderId = i;
            String orderContent = "jack" + orderId ;


            TOrder order = new TOrder();
            order.setOrder_id(orderId);
            order.setOrder_content(orderContent);
            commonMapper.addTOrderAndId(order);

            TOrderItem tOrderItem = new TOrderItem();
             tOrderItem.setOrder_item_id(SnowflakeUtil.nextId());
            tOrderItem.setOrder_id(orderId);
            commonMapper.addTOrderItem(tOrderItem);
        }
     ...
     * */
    @Test
    public void testQueryTOrderById() {
        TOrder order = new TOrder();
        order.setOrder_id(147);
        List<Map<String,Object>> mapList =  commonMapper.queryTOrderById(order);
     ...
     * */
    @Test
    public void testQueryTOrderItem() {
        TOrder order = new TOrder();
        order.setOrder_id(103);
        List<Map<String,Object>> mapList =  commonMapper.queryTOrderItem(order);
        System.out.println(mapList);
    }
